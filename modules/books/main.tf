...
resource "null_resource" "books_image" {
  ...
  provisioner "local-exec" {
    command = "docker login -u ${local.docker_hub_username} -p ${local.docker_hub_password} && docker push ${local.docker_hub_username}/books:1.0.0"
  }
...
resource "google_cloud_run_v2_service" "books" {
  name     = "books"
